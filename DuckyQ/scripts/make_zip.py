...
def  main():
  ...
  installer = "\installer\DuckyQ Installer.exe"
   
  if demo:
    installer = "\installer\DuckyQ Demo Installer.exe"
   
  FILES_TO_ZIP = [
    projectpath + installer,
    projectpath + "\installer\changelog.txt",
    projectpath + "\installer\known-issues.txt",
    projectpath + "\manual\DuckyQ manual.pdf" 
  ]
  ...
  ZIPNAME = "DuckyQ-v" + FULLVERSIONSTR + "-win.zip"
  
  if demo:
    ZIPNAME = "DuckyQ-v" + FULLVERSIONSTR + "-win-demo.zip"
