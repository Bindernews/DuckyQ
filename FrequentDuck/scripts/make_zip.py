...
def  main():
  ...
  installer = "\installer\FrequentDuck Installer.exe"
   
  if demo:
    installer = "\installer\FrequentDuck Demo Installer.exe"
   
  FILES_TO_ZIP = [
    projectpath + installer,
    projectpath + "\installer\changelog.txt",
    projectpath + "\installer\known-issues.txt",
    projectpath + "\manual\FrequentDuck manual.pdf" 
  ]
  ...
  ZIPNAME = "FrequentDuck-v" + FULLVERSIONSTR + "-win.zip"
  
  if demo:
    ZIPNAME = "FrequentDuck-v" + FULLVERSIONSTR + "-win-demo.zip"
